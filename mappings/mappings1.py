...
stub = modal.Stub("mappings1")
q = Queue()
pbar_lock = Lock()
# Fetch the gene description from NCBI Entrez

API_KEY = "0808d73c5f668b93c1ba52ee819a99df8c09"
YOUR_JSON_PATH = '/Users/danielgeorge/Documents/work/ml/bioconceptvec-explorer/bioconceptvec-explorer/embeddings/concept_glove_1.json'
...
def fetch_ncbi_species(id):
    url = f"https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esummary.fcgi?db=taxonomy&id={id}&api_key={API_KEY}"
...
def fetch_dbsnp(rs_id):
    url = f"https://api.ncbi.nlm.nih.gov/variation/v0/beta/refsnp/{rs_id}&api_key={API_KEY}"
    response = requests.get(url)
    if response.status_code == 200:
...
def main():
    ...
    def update_concept_description(concept):
        concept_descriptions.update(
            {concept: fetch_concept_description(concept)})
    ...
    # Save to pickle file
    with open('/Users/danielgeorge/Documents/work/ml/bioconceptvec-explorer/bioconceptvec-explorer/datasets/concept_descriptions_1.pkl', 'wb') as f:
